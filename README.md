...
- `src/routes/sources.ts` — Source CRUD + job enqueue + summary fetch (D-068) + `POST /:id/text` (D-150/D-065): the text-file **skip-transcription** ingest path. Writes the client-posted raw text onto the source's `transcript` attribute (where `heediq-worker-summarization` reads it — `contentRef` IS the `sourceId`, not an S3 key), flips the row to `status: 'processing'` / `sourceType: 'text'`, then enqueues a `SummarizationJobMessage` (`{ jobId, sourceId, orgId, sourceType: 'text', contentRef, tier }`) straight onto the summarization SQS queue — the transcription pipeline is never touched. No tier-gating (text has no model choice; `tier` is read only to stamp the job body for the summarizer's Claude-model routing — carried in the message body, no SQS message attribute, D-157). The transcript write is `attribute_exists(sourceId)`-conditioned (404 on a create→delete race); if the enqueue fails after the write, the source is flipped to `failed` so it never sits stuck in `processing`. Gated `requirePermission('sources:create')`. `+ POST /:id/review` (D-143/D-144): files approved extracted items into a Context. After the kept items are committed and audited, the route best-effort enqueues a `LedgerJobMessage` (`{ jobId, contextId, sourceId, orgId, tier }`) onto the ledger SQS queue for `heediq-ledger` to reconcile (D-148) — only when `keptCount > 0` (nothing kept ⇒ nothing to reconcile), tier resolved via the same `heediq-orgs`-`plan` lookup used for chat/jobs. The enqueue is wrapped so a queue failure logs but never fails the already-committed review.
...
- `src/routes/upload.ts` — `POST /api/v1/upload/presign` (S3 presigned URL). Before minting the URL it stamps the intended `audioS3Key` (`sources/{orgId}/{sourceId}/audio`) + `sourceType: 'audio'` onto the source row (D-150) — the audio-upload counterpart to `POST /:id/text`. This is what later gates `POST /:id/jobs` (whose transcribe enqueue requires `audioS3Key`). The stamp is `attribute_exists(sourceId)`-conditioned, so presigning against an unknown or other-org `sourceId` 404s here rather than issuing a URL for a key nothing owns.
...
- `src/handlers/auth-trigger-post-authentication.ts` — Cognito PostAuthentication trigger (`PostAuthentication_Authentication` only): resolves the canonical `accountId` the same way `auth-provision.ts` will moments later (identities table first, D-099), records the auth method for the login just completed, self-heals by pinning an unresolved `sub` via `linkIdentity`, and auto-links a federated login to an existing native account with the same email if not yet linked. Also emits the server-side `login_completed {method}` analytics event (D-154): `method` = the federated provider name lowercased (`google`/`microsoft`) when the current identity is external and its provider context is present, else `'password'`; validated against `AnalyticsAuthMethodSchema` before emitting. It reads `orgId` from `heediq-users` (one extra `GetCommand`, negligible against the 5s trigger budget) and **skips a genuinely-first login** — the user row doesn't exist yet at PostAuthentication time (it's written moments later by `auth-provision.ts`), so that login is represented by `user_provisioned` instead. PostAuthentication (not PreTokenGeneration) is the emit site because it fires only on real authentication, never token refresh, so logins aren't over-counted.
...
- `src/handlers/ws-pusher.ts` — DDB Streams (`MODIFY`) consumer on `heediq-jobs` (D-061, generalized D-109): builds a `job_status` event via `@heediq/shared`'s `buildWsEvent()` and pushes it at org scope via `wsPush.ts`. On a **terminal** job status (`done`/`failed`) it additionally emits the server-side `source_processing_completed {sourceId, jobId, status}` analytics event (D-154): a read-only `GetCommand` on `heediq-sources` (`SOURCES_TABLE_NAME`, keyed `{orgId, sourceId}`) resolves the uploader's `userId` (only `SourceSchema` carries it, not `JobSchema`), then `emitServerAnalytics` fires under that identity. Wrapped so a lookup/emit failure logs but never breaks the WS push. This is the one place with a `jobId`, a terminal status, and a cheap `userId` lookup — hence the chosen server emit site (joins a client `capture_started {sourceId}` on `sourceId`).
...
- `src/lib/analytics.ts` — server-side Amplitude choke-point (D-154): `emitServerAnalytics(input)` wraps `@heediq/shared`'s `buildServerAnalyticsEvent` (the pure builder — shared with the client so join keys can't drift) around the Amplitude Node HTTP V2 SDK (`@amplitude/analytics-node`). Maps the envelope to a `track()` call with `user_id = accountId` (D-099, never `sub`/`email`), `groups: { org: orgId }`, `insert_id` = the builder's deterministic hash (so at-least-once Lambda/DDB-stream retries never double-count), `time` from the domain-event timestamp, and id/enum/count-only `event_properties` (D-093). **Fail-safe & latency-bounded by construction:** a missing `AMPLITUDE_API_KEY` is a clean no-op; `init` runs once and is cached; `buildServerAnalyticsEvent`/`track` errors are caught and logged, never rethrown (so it can't break the login-critical auth triggers or a WS push); the post-`track` flush is bounded by `Promise.race` against an 800 ms timer so a stalled Amplitude endpoint can't hold a short-lived Lambda open. The only three emitters are the terminal-status branch of `ws-pusher.ts` and the two auth triggers below.
- `src/lib/analytics.ts` gotcha — the shared `AnalyticsIdentitySchema` validates `orgId` as a **UUID** at runtime even though the emitted `.d.ts` widens it to `string`; a non-UUID `orgId` makes `buildServerAnalyticsEvent` throw (caught → the event is silently dropped). Every emit site therefore passes the real `custom:orgId` UUID.
...
Client  →  API Gateway HTTP API  →  Lambda (Hono)
       ...
       /sources/:id/text  →  org-keyed source gate + DynamoDB (transcript write) + SQS (summarization queue) — skip-transcription path (D-150/D-065)
...
POST   /api/v1/sources/:id/text       { text }  -> { jobId }  (skip-transcription ingest, D-150/D-065, requires sources:create)
...
`MessageAttributes` — the worker reads the tier off the message body itself, same as every other
queue consumer since D-157); `heediq-chat` (a separate worker repo) consumes
...
```bash
pnpm run test          # 309 unit tests (auth routes + auth methods + settings link + auth triggers (incl. D-154 login_completed emit) + sources (incl. D-148 ledger enqueue) + contexts (incl. cross-org grant access) + context-grants + ledger (D-136/D-148 CRUD) + conversations (D-138/D-139 chat + D-149 gating) + app routing + rate limiting + roles + groups + role-assignments + rbac + rbac-middleware + me + users + wsPush + ws-connect + ws-pusher (incl. D-154 source_processing_completed emit) + classification-pusher + analytics (D-154 server emit helper))
